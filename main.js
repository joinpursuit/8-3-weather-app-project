...
form.addEventListener("submit", async (event) => {
    ...
    await fetch(`https://wttr.in/${userInput}?format=j1`)
    ...
    .then(climate => {
        ...
        const datas = [userInput, area, region, country, feelsLikeF, chanceSunshine, chanceRain, chanceSnow];
        ...
        function listOfPreviousSearches(area, feelsLikeF) {
            ...
            if (!unordered.innerHTML.includes(area)) {
                ...
                links[links.length-1].addEventListener("click", event => {
                    ...
                    searchResult(namesOfLocation[0], area, region, country, feelsLikeF, chanceSunshine, chanceRain, chanceSnow);
                    threeDaysWeather(temp);
                    listOfPreviousSearches(area, feelsLikeF);
                    convertTemperature();
                })
